...
final class Connection_Page {
	...
	private function handoff_prompt( string $base_url ): string {
		return "Use this WordPress site through Magick AI Adapter.\n"
			...
			. "Poll GET /proposals/{proposal_id} for Core status. For the unified user action, call POST /proposals/{proposal_id}/approve-and-execute so Adapter calls Core approve, Core commit-preflight, and one allowlisted execution. If status=rejected, stop and show the rejection status. If status=approved and execution is intended, call POST /proposals/{proposal_id}/execute. Use Adapter commit-preflight only as an advanced diagnostic step and follow it immediately with Adapter execute.\n"
	...
	private function openclaw_created_handoff_text( string $username, string $password_uuid, bool $include_local_tls ): string {
		return "Magick AI Adapter OpenClaw connection\n"
			...
			. "7. For the unified user action, call POST /proposals/{proposal_id}/approve-and-execute. Adapter calls Core approve, Core commit-preflight, and one allowlisted execution. Current execution allowlist: magick-ai/trash-post, magick-ai/create-draft, magick-ai/update-post, magick-ai/patch-post-content, magick-ai/patch-setting-value, magick-ai/set-post-seo-meta, magick-ai/set-post-slug, magick-ai/set-post-terms, magick-ai/delete-term, magick-ai/update-media-details, magick-ai/upload-media-from-url, magick-ai/set-post-featured-image, magick-ai/delete-media-permanently, magick-ai/reply-comment, magick-ai/trash-comment, magick-ai/approve-comment.\n"
			. "7b. If status=rejected, stop and show the rejection status. If status=approved and execution is intended, call POST /proposals/{proposal_id}/execute. Use Adapter commit-preflight only as an advanced diagnostic step and follow it immediately with Adapter execute.\n"
