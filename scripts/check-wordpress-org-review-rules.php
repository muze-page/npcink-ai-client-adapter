...
function npcink_openclaw_adapter_wporg_fail( $message ) {
	global $failures;
	$failures[] = (string) $message;
}

function npcink_openclaw_adapter_wporg_read( $path ) {
	if ( ! is_readable( $path ) ) {
		npcink_openclaw_adapter_wporg_fail( 'Missing readable file: ' . $path );
		return '';
	}
...
function npcink_openclaw_adapter_wporg_php_files() {
	global $root;
...
foreach ( npcink_openclaw_adapter_wporg_php_files() as $file ) {
	...
	$contents = npcink_openclaw_adapter_wporg_read( $file );

	foreach ( $rules as $message => $pattern ) {
		if ( preg_match( $pattern, $contents ) ) {
			npcink_openclaw_adapter_wporg_fail( $relative . ': ' . $message );
		}
	}
}
