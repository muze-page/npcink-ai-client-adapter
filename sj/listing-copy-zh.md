...
面向 Npcink Governance Core 治理和 WordPress Abilities API 路由的薄 OpenClaw 适配器。
...
在产品化 OpenClaw 配置中，OpenClaw 连接 Adapter。Npcink Governance Core 仍然是
proposal storage、approval、commit preflight 和 audit 的治理后端。Npcink
...
- 将受治理的写入请求转发到 Npcink Governance Core proposal routes。
...
- 需要把 OpenClaw 连接到 Npcink 站点的 WordPress 管理员。
...
- WordPress 7.0 或更高版本。
- PHP 8.0 或更高版本。
- Npcink Governance Core 用于治理。
- 一个 WordPress Abilities API provider，例如 Npcink Abilities Toolkit，用于能力定义和 callbacks。
