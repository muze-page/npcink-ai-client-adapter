...
REPORT_DIR="${MAA_ADAPTER_DEV_ARTICLE_VISUAL_REPORT_DIR:-$ROOT_DIR/build/dev-article-template-visual}"
MANIFEST_PATH="${MAA_ADAPTER_VISUAL_ACCEPTANCE_OUT:-$REPORT_DIR/manifest.json}"
BACKUP_PATH="${MAA_ADAPTER_DEV_ARTICLE_VISUAL_BACKUP:-$REPORT_DIR/template-backup.json}"
PLAN_REPORT_PATH="${MAA_ADAPTER_DEV_ARTICLE_VISUAL_PLAN_REPORT:-$REPORT_DIR/plan-report.json}"
KEEP_TEMPLATE="${MAA_ADAPTER_DEV_ARTICLE_VISUAL_KEEP_TEMPLATE:-}"
...
restore_template() {
	if [[ "$KEEP_TEMPLATE" == "1" || "$KEEP_TEMPLATE" == "true" ]]; then
		echo "Keeping local article template visual candidate in place because MAA_ADAPTER_DEV_ARTICLE_VISUAL_KEEP_TEMPLATE=$KEEP_TEMPLATE"
		return
	fi
	if [[ -f "$BACKUP_PATH" ]]; then
		MAA_ADAPTER_DEV_ARTICLE_VISUAL_MODE=restore \
		MAA_ADAPTER_DEV_ARTICLE_VISUAL_BACKUP="$BACKUP_PATH" \
		run_wp eval-file "$ROOT_DIR/tests/dev-article-template-visual.php" >/dev/null || true
	fi
}
...
MAA_ADAPTER_DEV_ARTICLE_VISUAL_BACKUP="$BACKUP_PATH" \
MAA_ADAPTER_DEV_ARTICLE_VISUAL_PLAN_REPORT="$PLAN_REPORT_PATH" \
run_wp eval-file "$ROOT_DIR/tests/dev-article-template-visual.php"
